...
var Influx = require('influx'),
    ...
    impl = {
            ...
            function setFromConfigOrEnv(configName, envName) {
                ...
                if (!pluginConfig[constants.CONFIG_INFLUX][configName]) {
                    ...
                    if (process.env[envName]) {
                        ...
                        throw new Error(util.format(messages.pluginParamOrEnvIsRequired, constants.CONFIG_INFLUX + '.' + configName, envName));
                    }
                }
            }
        ...
        validateConfig: function (scriptConfig) {
            ...
            if (!scriptConfig) {
                throw new Error(util.format(messages.pluginConfigIsRequired, constants.PLUGIN_NAME));
            }
            ...
            if (!scriptConfig[constants.CONFIG_TEST_NAME]) {
                throw new Error(util.format(messages.pluginParamIsRequired, constants.CONFIG_TEST_NAME));
            }
            ...
            // If no testRunId is provided in the static tags, generate one.
            ...
            requiredInfluxConfigs.forEach(function(configName) {
                if (!scriptConfig[constants.CONFIG_INFLUX][configName]) {
                    throw new Error(util.format(messages.pluginParamIsRequired, constants.CONFIG_INFLUX + '.' + configName));
                }
            });
        ...
        reportResults: function (influxReporter, testReport) {
            ...
            while (samples < testReport.latencies.length) {
                ...
                }, impl.config[constants.CONFIG_STATIC_TAGS])]);
            }

            influxReporter.writePoints(impl.config[constants.CONFIG_MEASUREMENT_NAME], points, function (err) {
                if (err) {
        ...
        reportErrors: function (influxReporter, testReport) {
            var errorCount = 0,
                points = [];

            // If there are no errors or error measurement name not defined (or empty), then exit.
            if (!testReport.errors || !impl.config[constants.CONFIG_STATIC_TAGS]) {
                return;
            }
            ...
            points.push([{
                time: Date.now(),
                value: errorCount
            }, Object.assign({}, impl.config[constants.CONFIG_STATIC_TAGS])]);

            // If errors were found, log them.
            if (points.length > 0) {
                influxReporter.writePoints(impl.config[constants.CONFIG_ERROR_MEASUREMENT_NAME], points, function (err) {
                    if (err) {
                        throw new Error(err.message);
                    } else {
                        console.log(util.format(messages.metricsReportedToInflux, points.length));
    ...
    api = {
        init: function (scriptConfig, eventEmitter) {
            ...
            eventEmitter.on('stats', function (report) {
                ...
                impl.reportErrors(reporter, report);
            });
        }
    };
